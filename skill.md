...
PDFgo 是一个面向律师阅卷和律师本地知识库部署的强力 skill。它把 PDF、扫描件、图片、Word、Excel、音频和视频材料转换为可追溯的 Markdown，既方便律师复核卷宗，也可以作为律所/律师个人本地知识库、Local Wiki、RAG 和智能体文档库的解析入口。

PDFgo 的输出天然适配 Obsidian、Notion、Logseq、Local Wiki、本地 LLM 知识库和 RAG 工作流。解析后的卷宗 Markdown 可以直接放入资料库，按案件、证据、人物、时间线建立链接，方便全文检索、双链整理、标签管理、向量索引和后续智能体阅读。
...
- 部署成本地 LLM 知识库、Local Wiki、RAG 或 Agent 文档库
...
6. 适合被本地嵌入模型、向量数据库、LLM Wiki 或 RAG 系统索引
```
...
Markdown 阅卷材料
  ↓
本地 Wiki / Obsidian / Notion / Logseq
  ↓
本地向量索引 / RAG / Agent / LLM 知识库
```

这样做的好处是：卷宗材料不出本机，Markdown 可审计、可检索、可复核，也方便以后接入本地大模型或小模型知识库。
...
```text
PDFgo = 律师本地阅卷解析 + 本地法律知识库部署的强力 skill
```
